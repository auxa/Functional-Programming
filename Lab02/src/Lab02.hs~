...
pathDistance p = if null p
			then 0
		else(distance (head p)) + (pathDistance (tail p))
...
pathTime p = if length p<1
	then 0
	else(time (head p)) + (pathTime (tail p))
...
getMeasured rqdDist p = if length p < 2 then p
			else if pathDistance p > rqdDist && pathDistance (init p) < rqdDist then p
			else getMeasured rqdDist (init p)
...
findFastest rqdDist p  =  if pathDistance(p) < rqdDist then Nothing
				else if pathDistance(p) == rqdDist
					then Just(p, pathSpeed(p))
				else if pathSpeed(getMeasured rqdDist p) > pathSpeed(getMeasured rqdDist (tail p))
					then Just(getMeasured rqdDist p, pathSpeed(getMeasured rqdDist p))
				else findFastest rqdDist (tail p)
